...
class Board:
    def __init__(self, N, is_cluster=True, _lambda = 0 ):
        ...
        if is_cluster:
            self.output_dir = '/cs/labs/nati/tamarals/Metropolis_Check_Progress/Metropolis_Markov_Chain_Priority/'
        else:
    ...
    def choose_good_random_cell(self):
        if len(self.good_cells_count) == 0:
            return  (-1,-1)

        good_cells_list = []
        for num in self.good_cells_count:
            for cell in self.good_cells_count[num]:
                good_cells_list.append(cell)

        cell = random.choice(good_cells_list)
        return cell

    def choose_good_random_cell_numSymbols(self):
        if len(self.good_cells_count) == 0:
            return  (-1,-1)

        num = min(self.good_cells_count)
        cell = random.choice(self.good_cells_count[num])
        return cell
