...
class Board:
    def __init__(self, N, is_cluster=True, _lambda = 0 ):
        ...
        if is_cluster:
            self.output_dir = '/cs/labs/nati/tamarals/toroidal_updates/couples_changezero_absorbers/'
        else:
    ...
    def switch_empty_cell_with_one_or_two(self):
        cell = random.choice(self.empty_cells)
        symbols_list = self[cell].symbols_with_one_or_two_threat()
        if len(symbols_list) == 0:
            return False

        symbol = random.choice(symbols_list)
        switch_cells = copy.deepcopy(self[cell].bad_symbols[symbol])
